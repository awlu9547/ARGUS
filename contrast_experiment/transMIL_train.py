...
from contrast_experiment.multiMIL_metrics import val_auc
from contrast_models.m_UNI_model import m_UNI
...
if __name__ == '__main__':
    ...
    model = m_UNI(n_classes=num_classes, freeze_ratio=args.freeze_ratio, cmb=args.cmb, ckpt_path=args.UNI_path)
    model.to(device)
        ...
        for iter_idx, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            if ((args.iters_to_val != -1 and iter_idx % args.iters_to_val == 0 and iter_idx != 0)
                ...
                result = val_auc(model=model, loader=val_loader, criterion=criterion_val,
                                 ...
                                 iter_idx=iter_idx, mil_method='transformer')
