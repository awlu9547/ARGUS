...
from .multi_scale_feature_model import MultiScaleModel_1, MultiScaleModel_4, MultiScaleModel_5, MultiScaleModel_6, \
    MultiScaleModel_7, \
    MultiScaleModel_8

from .gcn import GCNModel
from .modeling_catfusion import CATfusion
...
class MSUNI(nn.Module):
    def __init__(self, n_classes, freeze_ratio, cmb, ckpt_path, use_multiscale=True):
        ...
        self.configs = CONFIGS['CATfusion']
        ...
        if use_multiscale:
            ...
            self.MSfusion = CATfusion(config=self.configs, num_classes=n_classes, zero_head=True)
...
class MSUNI_Ablation_Catfusion(nn.Module):
    def __init__(self, n_classes, freeze_ratio, cmb, ckpt_path, use_multiscale=True):
        super().__init__()
...
class MSUNI_Ablation_MSfusion(nn.Module):
    def __init__(self, n_classes, freeze_ratio, cmb, ckpt_path, use_multiscale=True):
        super().__init__()
...
if __name__ == '__main__':
    ...
    config = configs.get_CATfusion_config()
    n_classes = 3
    batch_size = 5
    patch_dim = 1536

    model = MSUNI_test(n_classes=n_classes, use_multiscale=True)
