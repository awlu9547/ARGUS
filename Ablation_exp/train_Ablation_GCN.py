...
from ARGUS_utils.dataset import GCNDatasetKFold
from ARGUS_utils.metrics import val_GCN_branch
from ARGUS_models.main_model import GCN_branch
import numpy as np
import random


def load_config(config_path=os.path.join(os.path.dirname(os.path.dirname(__file__)), 'config.yaml')):
...
def train_collate_fn(batch):
    label, feature_matrix, cooadj_matrix = zip(*batch)
    return (
...
def val_collate_fn(batch):
    slide, label, feature_matrix, cooadj_matrix = zip(*batch)
    return (
        list(slide),
        torch.stack(label),  # [batch_size]
        list(feature_matrix),  # [batch_size, 1536]
...
if __name__ == '__main__':
    ...
    assert os.path.exists(
        '<YOUR_KF_PATH>'), 'k-fold dir not exists, run utils/gen_kfold_split.py first'
    print("check kf file successfully")

    # create dataset objects for train and val
    train_dataset = GCNDatasetKFold(mode='train', fold=args.fold)
    val_dataset = GCNDatasetKFold(mode='val', fold=args.fold)
    if train_dataset and val_dataset is not None:
        print("load dataset successfully")

    # create data loaders for train and val
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=10, collate_fn=train_collate_fn)
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=15, collate_fn=val_collate_fn)
    if train_loader and val_loader is not None:
        print("load loader successfully")

    model = GCN_branch(n_classes=num_classes)
    model.to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for iter_idx, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            label, feature_list, cooadj_list = batch
            ...
            output = model(feature_list, cooadj_list)
            ...
            if ((args.iters_to_val != -1 and iter_idx % args.iters_to_val == 0 and iter_idx != 0)
                ...
                result = val_GCN_branch(model=model, loader=val_loader, criterion=criterion_val,
                ...
                # Update log file
                log_data = {'epoch': epoch + 1, 'iter_idx': iter_idx, 'slide_auc': slide_AUC, 'slide_accuracy': slide_ACC,
                ...
                if not os.path.exists(log_file_path):
                    pd.DataFrame(columns=['epoch', 'iter_idx', 'slide_auc', 'slide_accuracy', 'slide_F1', 'slide_precision',
