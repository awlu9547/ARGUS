...
from MSUNI_utils.metrics import val_Ablation_MSfusion
from MSUNI_models.main_model import MSUNI_Ablation_MSfusion
import random
import numpy as np
...
if __name__ == '__main__':
    ...
    model = MSUNI_Ablation_MSfusion(n_classes=num_classes, freeze_ratio=args.freeze_ratio, cmb=args.cmb, ckpt_path=args.UNI_path)
    model.to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for iter_idx, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            if ((args.iters_to_val != -1 and iter_idx % args.iters_to_val == 0 and iter_idx != 0)
                ...
                result = val_Ablation_MSfusion(model=model, loader=val_loader, criterion=criterion_val,
