...
from MSUNI_utils.dataset import PathologyDatasetKFold
from MSUNI_utils.metrics import val_Ablation_Catfusion
from MSUNI_models.main_model import MSUNI_Ablation_Catfusion
import random
import numpy as np
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train UNI")
...
if __name__ == '__main__':
    ...
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=12, collate_fn=val_collate_fn)
    if train_loader and val_loader is not None:
        print("load loader successfully")

    model = MSUNI_Ablation_Catfusion(n_classes=num_classes, freeze_ratio=args.freeze_ratio, cmb=args.cmb, ckpt_path=args.UNI_path)
    model.to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for iter_idx, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            output = model(tile, feature_list, cooadj_list)
            ...
            if ((args.iters_to_val != -1 and iter_idx % args.iters_to_val == 0 and iter_idx != 0)
                ...
                result = val_Ablation_Catfusion(model=model, loader=val_loader, criterion=criterion_val,
