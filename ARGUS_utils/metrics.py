...
def val_Ablation_GPGF(model, loader, criterion, class_names, save_path, epoch, iter_idx, save_as_pdf=False):
    print(f'Validating: iter {iter_idx} ...')
    tic = time.time()
    num_classes = len(class_names)
    device = next(model.parameters()).device
...
def val_Ablation_HFA(model, loader, criterion, class_names, save_path, epoch, iter_idx, save_as_pdf=False):
    print(f'Validating: iter {iter_idx} ...')
    tic = time.time()
    num_classes = len(class_names)
    device = next(model.parameters()).device
